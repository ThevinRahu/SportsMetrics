...
export default async function handler(req, res) {
  ...
  await sql`
    UPDATE tournaments 
    SET teams = ${JSON.stringify(teams)}::jsonb, round = 3,
  ...
  res.status(200).json({
    success: true, matchesReset: resetMatches.length,
    matches: resetMatches.map(m => `${m.home_team} vs ${m.away_team}`),
    message: 'NC2026 reset to R2 complete state. Ready for R3 extraction test.'
  });
}
